...
# 2. 获取所有课程
echo -e "\n2. 获取所有课程"
curl -s http://localhost:8081/api/courses | json_pretty
echo ""
...
# 4. 获取所有学生
echo -e "\n4. 获取所有学生"
curl -s http://localhost:8082/api/students | json_pretty
echo ""
...
if [ -z "$COURSE_ID" ] || [ "$COURSE_ID" == "null" ]; then
    ...
    # 使用-i参数显示响应头和响应体
    curl -i -X POST "http://localhost:8082/api/enrollments?courseId=$COURSE_ID&studentId=2024001"
fi
echo ""

# 6. 查询选课记录
echo -e "\n6. 查询选课记录"
curl -s http://localhost:8082/api/enrollments | json_pretty
echo ""

# 7. 测试选课失败（课程不存在）
echo -e "\n7. 测试选课失败（预期返回404 Not Found）"
# 【关键修改】使用 -i 参数来打印完整的HTTP响应，包括状态码和头部信息
curl -i -X POST "http://localhost:8082/api/enrollments?courseId=non-existent-course&studentId=2024001"
echo ""
